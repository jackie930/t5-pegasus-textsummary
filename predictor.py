...
def pegasus_infer(checkpoint_path,dict_path,cfg_path,text,weight_path):
    max_c_len = 500
    max_t_len = 200
    ...
    print ("<<<<done")
    return result,str(t2-t1),str(t3-t2)
...
#     s3_client.download_file(bucket, image_uri, download_file_name)
    ...
    for i in [cfg_path,checkpoint_path,dict_path,weight_path]:
        if os.path.exists(i):
    ...
    #make inference
    res,load_time,infer_time = pegasus_infer(checkpoint_path,dict_path,cfg_path,text,weight_path)
